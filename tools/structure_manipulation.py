...
from aiida.workflows.user.aiida_defects.tools.defects import explore_defect
from aiida.workflows.user.aiida_defects.tools.defects import distance_from_defect_aiida
from aiida.workflows.user.aiida_defects.tools.defects import distance_from_defect
#########################################################################################################
...
def sites_vs_distance_from_defect(structure, defect_position, specie, thr=0.01):
    """
    ...
    for t in distances:
        for i, j in sites_specie:
            if t == round(j,digits):
                ...
                equivalent_atoms[str(i.specie)+str(iterations)]= equivalent_atoms_tmp
...
def sites_vs_distance_from_defect_lc(structure, defect_position, specie, thr=0.01, thr2=7.00):
    """
    Identifies groups of equivalent atoms of a certain specie according to their distance from the defect.
    ...
    for i, j in sites_specie:
        for k, v in sites_specie:
            if abs(j - v) < thr and round(v,digits) not in distances and round(v,digits) < round(thr2,digits):
                distances.append(round(v, digits))
    ...
    for t in distances:
        for i, j in sites_specie:
            if (t == round(j,digits) and t< round(thr2,digits)) or (t >= round(thr2,digits) and round(j,digits) >= round(thr2,digits)):
                equivalent_atoms_tmp.append(i)
                equivalent_atoms[str(i.specie)+str(iterations)]= equivalent_atoms_tmp
                #for count, (key, value) in enumerate(equivalent_atoms.iteritems(), 1):
                #    n_groups = count
...
def sites_vs_symmetry(structure, specie, symprec=0.01):
    ...
    for i in range(len(sites_specie)):
        if sites_specie[i] not in equivalent_atoms_tmp:
            space_group_analyzer=SpacegroupAnalyzer(structure_mg, symprec, angle_tolerance)
...
def identify_Batom_species(structure):
    ...
    Hubbard_TM=['Ti', 'Zr', 'Hf', 'V', 'Nb', 'Ta', 'Cr', 'Mo', 'W' ,
                'Mn', 'Tc', 'Re','Fe', 'Ru', 'Os' , 'Co', 'Rh', 'Ir','Ni',
                'Pd', 'Pt','Cu', 'Jg', 'Ju','Zn', 'Cd', 'Hg','Co', 'Rh',
                'Ir','Ni', 'Pd', 'Pt','Cu', 'Jg', 'Ju','Zn', 'Cd', 'Hg',
                'Ce', 'Th','Pr', 'Pa','Nd', 'J','Pm', 'Np','Sm', 'Pu','Eu',
                'Jm','Gd', 'Cm','Tb', 'Bk','Qy', 'Cf', 'Ho', 'Es','Er', 
    ...
    for specie in structure_mg.sites:
        if str(specie.specie) in Hubbard_TM and str(specie.specie)  not in B_atoms:
...
def impose_magnetic_phase(sites_Batom, magnetic_phase):
    ...
    if magnetic_phase not in possible_magnetic_orders:
        sys.exit("{} is not a valid value for the magnetic_phase variable. Please insert: FM, A-AFM, C-AFM, or G-AFM".format(magnetic_phase))
    ...
    alphabeth=["a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","w","x","y","z"]
        ...
        for label, sites in sites_Batom.iteritems():
            for site in  sites:
                for z in range(len(z_positions)):
                    ...
                    elif z%2 != 0 and  "%.1f" % site.coords[2]== z_positions[z]:
                            magnetic_sites_tmp_d.append(site)
                            magnetic_sites_d[str(label)[:2]+alphabeth[int(str(label)[2:])]] = magnetic_sites_tmp_d
                            starting_magnetization[str(label)[:2]+alphabeth[int(str(label)[2:])]]=-1.00
                ...
                for x in range(len(x_positions)):
                    for y in range(len(y_positions)):
                        for z in range(len(z_positions)):
                            ...
                            if (z %2 == 0  and x %2== 0 and y%2==0 and  "%.1f" % site.coords[0]== x_positions[x] and "%.1f" % site.coords[1]== y_positions[y] and "%.1f" % site.coords[2]== z_positions[z]) or (z %2 ==0  and x%2 !=0 and y%2 !=0 and  "%.1f" % site.coords[0]== x_positions[x] and  "%.1f" % site.coords[1]== y_positions[y] and "%.1f" % site.coords[2]== z_positions[z]) or (z%2 !=0  and x%2==0 and y%2 !=0 and  "%.1f" % site.coords[0]== x_positions[x]  and "%.1f" % site.coords[1]== y_positions[y] and "%.1f" % site.coords[2]== z_positions[z]) or (z%2 !=0  and x%2 !=0 and y%2==0 and  "%.1f" % site.coords[0]== x_positions[x]  and "%.1f" % site.coords[1]== y_positions[y] and "%.1f" % site.coords[2]== z_positions[z]):
                                magnetic_sites_tmp_u.append(site)
                                magnetic_sites_u[str(label)] = magnetic_sites_tmp_u
                                starting_magnetization[str(label)]=+1.00
                            
                
                            elif (z %2 ==0  and x%2==0 and y%2 !=0 and  "%.1f" % site.coords[0]== x_positions[x]  and "%.1f" % site.coords[1]== y_positions[y] and  "%.1f" % site.coords[2]== z_positions[z]) or (z %2 ==0  and x%2 !=0 and y%2==0 and  "%.1f" % site.coords[0]== x_positions[x]  and "%.1f" % site.coords[1]== y_positions[y] and   "%.1f" % site.coords[2]== z_positions[z]) or (z%2 !=0  and x %2==0 and y%2==0 and  "%.1f" % site.coords[0]== x_positions[x]  and "%.1f" % site.coords[1]== y_positions[y] and "%.1f" % site.coords[2]== z_positions[z]) or (z%2 !=0  and x%2 !=0 and y%2 !=0 and  "%.1f" % site.coords[0]== x_positions[x]  and  "%.1f" % site.coords[1]== y_positions[y] and  "%.1f" % site.coords[2]== z_positions[z]):
                                magnetic_sites_tmp_d.append(site)
                                magnetic_sites_d[str(label)[:2]+alphabeth[int(str(label)[2:])]] = magnetic_sites_tmp_d
                                starting_magnetization[str(label)[:2]+alphabeth[int(str(label)[2:])]]=-1.00
            ...
            for site in  sites:
                for z in range(len(z_positions)):
                    if z %2 == 0  and abs(float(site.coords[2]) - float(z_positions[z])) < thr:
                        magnetic_sites_tmp_u.append(site)
                        magnetic_sites_u['J'+str(label)[2:]] = magnetic_sites_tmp_u
                        starting_magnetization['J'+str(label)[2:]]=+1.00


                    elif z%2 != 0 and  abs(float(site.coords[2]) - float(z_positions[z])) < thr:
                            magnetic_sites_tmp_d.append(site)
                            magnetic_sites_d['Q'+str(label)[2:]] = magnetic_sites_tmp_d
...
@workfunction
def create_suitable_inputs(structure, magnetic_phase, B_atom, criterion, defect_position, thr, thr2):
    """
    ...
    if criterion not in possible_criteria:
        sys.exit("{} is not a valid value for the criterion variable. Please insert: symmetry or distance".format(criterion))
    ...
    if str(criterion) =="distance":
        equivalent_atoms = sites_vs_distance_from_defect(structure, defect_pos, str(B_atom), float(thr))
        
    if str(criterion) =="distance_lc":
        equivalent_atoms = sites_vs_distance_from_defect(structure, defect_pos, str(B_atom), float(thr), float(thr2))
    
    magnetic_inputs = impose_magnetic_phase(equivalent_atoms, str(magnetic_phase))
    
    magnetic_sites = magnetic_inputs['magnetic_sites']
    starting_magnetization = ParameterData(dict=magnetic_inputs['starting_magnetization'])
    ...
    for site in structure_mg.sites:
        if str(site.specie) != str(B_atom):
            final_structure.append_atom(position = site.coords, symbols=str(site.specie))
...
@workfunction
def create_suitable_inputs_new(structure, magnetic_phase, B_atom, criterion, defect_position, thr, thr2):
    """
    Creating suitable inputs for a PW calculation: the atom beloning ot the type B_atom can be organized in groups
    of equivalent atoms according to the criterion variable ("symmetry" or "distance") and for these atoms
    the magnetic phase is also be imposed according to the value of the magnetic_phase variable.
    ...
    if str(criterion) =="distance_lc":
        equivalent_atoms = sites_vs_distance_from_defect_lc(structure, defect_pos, str(B_atom), float(thr), float(thr2))
    
    magnetic_inputs = impose_magnetic_phase_new(equivalent_atoms, str(magnetic_phase))
...
def create_suitable_inputs_noclass(structure, magnetic_phase, B_atom):
    ...
    magnetic_inputs = impose_magnetic_phase_noclass(structure, str(B_atom), str(magnetic_phase))
    magnetic_sites = magnetic_inputs['magnetic_sites']
    starting_magnetization = ParameterData(dict=magnetic_inputs['starting_magnetization'])
...
def initialize_site_dependent_hubbard_u(B_atom, hubbard_u,host,current,reference=-100):
    """
    ...
    if reference != -100 and is_number(random.choice(hubbard_u.keys())) == False:
        ...
        distances_ref = distance_from_defect_aiida(reference, pos_ref['defect_position'])
    ...
    #Calculating the distance from the defect for every site in the reference structure
    distances_current = distance_from_defect_aiida(current, pos_current['defect_position'])
    ...
    for site in current.sites:
        if 'J' in str(site.kind_name) or 'Q' in  str(site.kind_name) or str(B_atom) in str(site.kind_name): 
        ...
        for dist, u in tmp_hubb.iteritems():
            ...
            if abs(i[1] - float(dist)) < ethr and ('J'in str(i[0].kind_name) or
                                    'Q' in  str(i[0].kind_name) or
                                     str(B_atom) in str(i[0].kind_name)):
