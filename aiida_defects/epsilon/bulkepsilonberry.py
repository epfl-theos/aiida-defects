...
class BulkEpsilonBerryWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.outline(
            cls.validate_inputs,
            if_(cls.epsilon_high_frequency)(cls.run_pw_e0),
            cls.run_pw_e1,
            cls.retrieve_dipole,
    ...
    def validate_inputs(self):
        ...
        if str(self.inputs.epsilon_type) != 'high-frequency' and str(
                self.inputs.epsilon_type) != 'low-frequency':
            print(str(self.inputs.epsilon_type) != 'high-frequency'
                  and str(self.inputs.epsilon_type) != 'low-frequency')
            self.abort_nowait(
                'Check the type of dielectric constant requested. Allowed values are: high-frequency and low-frequency'
            )

        elif self.inputs.epsilon_type == Str('low-frequency'):
            self.abort_nowait(
                'The calculation of the low-frequency dielectric constant within the Berry phase method is not yet implemented'
            )
    ...
    def run_pw_e0(self):
        ...
        self.report(
            'Launching PwBaseWorkChain for a FEF calculation with amplitude 0.0 a.u.. pk value {}'
            .format(running.pid))
    ...
    def run_pw_e1(self):
        ...
        running = submit(PwBaseWorkChain, **inputs)
        self.report(
            'Launching PwBaseWorkChain for a FEF calculation with amplitude {} a.u.. pk value {}'
    ...
    def compute_bulk_epsilon(self):
        ...
        if int(self.inputs.edir) == 1:
            bulk_epsilon = 4 * pi * (
                self.ctx.dipole_e1[0] - self.ctx.dipole_e0[0]) / (
                    float(self.inputs.eamp) * volume) + 1

        elif int(self.inputs.edir) == 2:
            bulk_epsilon = 4 * pi * (
                self.ctx.dipole_e1[1] - self.ctx.dipole_e0[1]) / (
                    float(self.inputs.eamp) * volume) + 1
        elif int(self.inputs.edir) == 3:
            bulk_epsilon = 4 * pi * (
                self.ctx.dipole_e1[2] - self.ctx.dipole_e0[2]) / (
                    float(self.inputs.eamp) * volume) + 1
        self.out('epsilon', Float(bulk_epsilon))
        self.report(
            "BulkEpsilonBerryWorkChain completed succesfully. The {} dielectric constant is {}"
            .format(self.inputs.epsilon_type, bulk_epsilon))
