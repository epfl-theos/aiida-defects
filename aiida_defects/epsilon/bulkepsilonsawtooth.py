...
class BulkEpsilonSawtoothWorkChain(WorkChain):
    ...
    def compute_bulk_epsilon(self):
        ...
        Vsaw = planar_average(
            self.ctx.grid_e1_saw, self.ctx.structure, axis, npt=400)
        Ve0 = planar_average(
            self.ctx.grid_e0, self.ctx.structure, axis, npt=400)
        Ve1 = planar_average(
            self.ctx.grid_e1, self.ctx.structure, axis, npt=400)
        ...
        self.report(
            "BulkEpsilonSawtoothWorkChain completed succesfully. The {} Epsilon value is {}"
            .format(self.inputs.epsilon_type, epsilon))
