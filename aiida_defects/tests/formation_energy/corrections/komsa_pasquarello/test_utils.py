...
class TestUtils(object):
    ...
    def test_get_cell_matrix(self, test_structures):
        ...
        reference_cell_matrix = np.array(
            [[10.305243520465277, 0., 0.], 
            [0., 10.305243520465277, 0.],
            [0.,0.,10.305243520465277]])
        np.testing.assert_array_almost_equal(calculated_cell_matrix, reference_cell_matrix, decimal=15)
    ...
    def test_get_reciprocal_cell(self):
        ...
        # Cubic cell
        real_space_cell = np.array([[10.0,0.,0.],[0.,10.,0.],[0.,0.,10.]])
        reference_cell = np.array([[2*np.pi/10,0.,0.],[0.,2*np.pi/10,0.],[0.,0.,2*np.pi/10]])
        calculated_reciprocal_cell = get_reciprocal_cell(real_space_cell)
        np.testing.assert_array_equal(calculated_reciprocal_cell, reference_cell)
        ...
        calculated_reciprocal_cell = get_reciprocal_cell(real_space_cell)
        np.testing.assert_array_almost_equal(calculated_reciprocal_cell, reference_cell, decimal=6)
    ...
    def test_get_reciprocal_grid(self):
        ...
        reciprocal_cell = np.array([[2*np.pi/10,0.,0.],[0.,2*np.pi/10,0.],[0.,0.,2*np.pi/10]])
    ...
    def test_get_charge_model(self):
        ...
        # Charge = -1 
        g = get_charge_model(limits, dimensions, position, sigma=1.0, charge=-1.0)
        calculated_charge = np.trapz(np.trapz(np.trapz(g, coords) ,coords), coords)
        reference_charge = -1.0
        np.testing.assert_array_almost_equal(calculated_charge, reference_charge, decimal=15)

        # Charge = -3 
        g = get_charge_model(limits, dimensions, position, sigma=1.0, charge=-3.0)
        calculated_charge = np.trapz(np.trapz(np.trapz(g, coords) ,coords), coords)
        reference_charge = -3.0
        np.testing.assert_array_almost_equal(calculated_charge, reference_charge, decimal=15)

        # Charge = -3 , sigma = 3.0
        g = get_charge_model(limits, dimensions, position, sigma=3.0, charge=-3.0)
        calculated_charge = np.trapz(np.trapz(np.trapz(g, coords) ,coords), coords)
        reference_charge = -3.0
        np.testing.assert_array_almost_equal(calculated_charge, reference_charge, decimal=15)
    ...
    def test_get_fft(self):
        ...
        g_k_ref = np.array([
            ...
            0.02065841+0.06358004j,  0.        +0.j        ,
            0.02065841-0.06358004j, -0.12525384+0.17239712j,
            0.38407615-0.27904766j, -0.72947318+0.2370202j  
        ])
    ...
    def test_get_energy(self):
        ...
        rho_r = get_charge_model(limits, dimensions, position, sigma=1.0, charge=-1.0)
