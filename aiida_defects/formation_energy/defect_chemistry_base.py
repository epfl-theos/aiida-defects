...
class DefectChemistryWorkchainBase(WorkChain):
    ...
    def define(cls, spec):
        ...
        # Correction workchain
        spec.input("correction_scheme", valid_type=orm.Str,
            help="The correction scheme to apply")
        spec.input("epsilon", valid_type=orm.Float, required=False,
            help="Dielectric constant of the host")

        # Outputs
#        spec.output(
#            "density_of_states", valid_type=orm.XyData, required=True)
        spec.output(
    ...
    def if_rerun_calc_unitcell(self):
        if not self.ctx['output_unitcell']:
            return True
#        elif self.ctx['calc_unitcell'].is_finished_ok:
#            is_insulator, band_gap = orm.nodes.data.array.bands.find_bandgap(self.ctx['calc_unitcell'].outputs.output_band)
#            self.ctx.vbm = get_vbm(self.ctx['calc_unitcell'])
#            self.ctx.number_of_electrons = self.ctx['calc_unitcell'].outputs.output_parameters.get_dict()['number_of_electrons']
...
#        if not self.ctx['calc_dos']:
#            #self.report('start from scratch')
#            return True
#        elif self.ctx['calc_dos'].is_finished_ok:
#            self.ctx.dos = self.ctx['calc_dos'].outputs.output_dos
#            #self.report('finished ok')
#            return False
#        else:
#            #self.report('not finished correctly')
#            return True
        if not self.ctx.dos:
            #self.report('start from scratch')
            return True
...
#        }
    ...
    def check_chemical_potential_workchain(self):
        ...
        for key, ef_dict in self.ctx.all_dopants.items():
            ...
            if not chem_potential_wc.is_finished_ok:
                ...
                self.ctx.chemical_potential[key] = chem_potential_wc.outputs.chemical_potential.get_dict()
                #self.ctx.stability_corners = chem_potential_wc.outputs.stability_corners
                #self.ctx.matrix_of_constraints = chem_potential_wc.outputs.matrix_of_constraints
        self.report('Chemical potentials: {}'.format(self.ctx.chemical_potential))
        self.out('chemical_potential', store_dict(orm.Dict(dict=self.ctx.chemical_potential)))
            ...
            for chg in properties['charges']:
                if chg != 0.0:
                    ...
                    if not correction_wc.is_finished_ok:
                        ...
                        self.ctx.total_correction[defect][str(chg)] = correction_wc.outputs.total_correction.value
                        self.ctx.electrostatic_correction[defect][str(chg)] = correction_wc.outputs.electrostatic_correction.value
                        self.ctx.potential_alignment[defect][str(chg)] = correction_wc.outputs.total_alignment.value
                else:
                    self.ctx.defect_data[defect]['charges']['0.0']['E_corr'] = 0.0
                    self.ctx.total_correction[defect]['0.0'] = 0.0
                    self.ctx.electrostatic_correction[defect]['0.0'] = 0.0
                    self.ctx.potential_alignment[defect]['0.0'] = 0.0
        self.out('defect_data', store_dict(orm.Dict(dict=self.ctx.defect_data)))
        self.out('total_correction', store_dict(orm.Dict(dict=self.ctx.total_correction)))
        self.out('electrostatic_correction', store_dict(orm.Dict(dict=self.ctx.electrostatic_correction)))
        self.out('potential_alignment', store_dict(orm.Dict(dict=self.ctx.potential_alignment)))
        self.report('The defect data are: {}'.format(self.ctx.defect_data))
    ...
    def run_fermi_level_workchain(self):
        ...
        inputs = {
            #"defect_data": orm.Dict(dict=self.ctx.defect_data),
            #"chem_potentials": self.ctx.chemical_potential,
            "temperature": self.inputs.temperature,
            "valence_band_maximum": orm.Float(self.ctx.vbm),
        ...
        intrinsic_defects = {}
        for defect, properties in self.ctx.defect_data.items():
            if is_intrinsic_defect(properties['species'], self.inputs.compound.value):
        ...
        for dopant in self.ctx.sc_fermi_dopants:
            ...
            inputs['defect_data'] = orm.Dict(dict=defect_temp)
            inputs['chem_potentials'] = orm.Dict(dict=self.ctx.chemical_potential[dopant])
    ...
    def check_fermi_level_workchain(self):
        ...
        self.report('Fermi level: {}'.format(self.ctx.fermi_level))
        self.out('fermi_level', store_dict(orm.Dict(dict=self.ctx.fermi_level)))
    ...
    def compute_defect_formation_energy(self):
        ...
        self.ctx.defect_formation_energy = get_defect_formation_energy(
                orm.Dict(dict=self.ctx.defect_data),
                #self.inputs.formation_energy_dict,
                #orm.Dict(dict=self.ctx.all_dopants),
                orm.Dict(dict=self.ctx.fermi_level),
                orm.Dict(dict=self.ctx.potential_alignment),
                orm.Dict(dict=self.ctx.chemical_potential),
                self.inputs.compound
                )

        self.report('The defect formation energy is :{}'.format(self.ctx.defect_formation_energy.get_dict()))
        self.out("defect_formation_energy", self.ctx.defect_formation_energy)
