...
from aiida_defects.formation_energy.new_defect_chemistry_base import DefectChemistryWorkchainBase
...
#        pw_inputs.code = self.inputs.qe.dft.unitcell.code
#        #pw_inputs.pseudos = self.inputs.qe.dft.unitcell.pseudopotentials
#        pw_inputs.pseudos = get_pseudos_from_structure(self.inputs.unitcell, self.inputs.qe.dft.unitcell.pseudopotential_family.value)
        
        kpoints = orm.KpointsData()
        kpoints.set_cell_from_structure(self.inputs.unitcell)
...
#        pw_inputs.kpoints = kpoints
        ...
        scheduler_options = self.inputs.qe.dft.unitcell.scheduler_options.get_dict()
        scheduler_options['label'] = 'unitcell'
#        pw_inputs.metadata = scheduler_options

#        pw_inputs.structure = self.inputs.unitcell
        parameters = self.inputs.qe.dft.unitcell.parameters.get_dict()
        #parameters['SYSTEM']['nbnd'] = self.inputs.nbands.value
#        pw_inputs.parameters = orm.Dict(dict=parameters)
    ...
    def check_unitcell_dft_calculation(self):
        ...
        if not unitcell_calc.is_finished_ok:
            ...
            self.ctx.vbm = get_vbm(unitcell_calc)
    ...
    def check_dos_calculation(self):
        ...
        if dos_calc.is_finished_ok:
            ...
            x = Dos.get_x()
            y = Dos.get_y()
            DOS = np.vstack((x[1]-self.ctx.vbm, y[1][1])).T
