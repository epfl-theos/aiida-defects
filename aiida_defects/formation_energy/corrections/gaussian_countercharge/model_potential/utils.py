...
@calcfunction
def get_charge_model(limits, dimensions, defect_position, sigma=orm.Float(1.0), charge=orm.Float(-1.0)):
    """
    ...
    print("DEBUG: Integrated charge density (scaled_g) = {}".format(get_integral(g, dimensions, limits)))

    #scaled_g = scaled_g - np.sum(scaled_g)/np.prod(scaled_g.shape)
    print("DEBUG: Integrated charge density (jellium) = {}".format(get_integral(g, dimensions, limits)))
...
def get_gaussian_3d(grid, position, sigma):
    ...
    gaussian = -np.exp(-(x**2+y**2+z**2)/(2*sigma**2))/((2.0*np.pi)**1.5*np.sqrt(sigma))
...
def get_model_potential(cell_matrix, dimensions, charge_density, epsilon):
    ...
    v_model = np.divide(charge_density_g, G_sqmod_array, where=G_sqmod_array!=0.0)
...
def get_energy(potential, charge_density, dimensions, limits):
    ...
    energy = np.real(0.5*np.trapz(np.trapz(np.trapz(charge_density*potential, ii), jj), kk)) 
