...
class GaussianCounterChargeWorkchain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input("model_iterations_required", valid_type=orm.Int, default=orm.Int(3))
        spec.input("epsilon", valid_type=orm.Float, 
        ...
        spec.output('model_correction_energies', valid_type=orm.List, required=True)
        spec.exit_code(401, 'ERROR_INVALID_INPUT_ARRAY',
            message='the input ArrayData object can only contain one array')
        spec.exit_code(409, 'ERROR_SUB_PROCESS_FAILED_ALIGNMENT',
            message='the electrostatic potentials could not be aligned')
        spec.exit_code(413, 'ERROR_SUB_PROCESS_FAILED_MODEL_POTENTIAL',
            message='The model electrostatic potential could not be computed')
        spec.exit_code(410, 'ERROR_SUB_PROCESS_FAILED_FINAL_SCF',
            message='the final scf PwBaseWorkChain sub process failed')
        spec.exit_code(411, 'ERROR_BAD_INPUT_ITERATIONS_REQUIRED',
    ...
    def setup(self):
        ...
        for arraydata in [self.inputs.v_host, self.inputs.v_defect_q0, self.inputs.v_defect_q]:
        ...
        self.ctx.v_defect_q0_array = v_defect_q0.get_array(v_defect_q0.get_arraynames()[0])
        
        v_defect_q = self.inputs.v_defect_q
        self.ctx.v_defect_q_array = v_defect_q.get_array(v_defect_q.get_arraynames()[0])
    ...
    def compute_model_potential(self):
        ...
        self.report("Computing model potential for scale factor {}".format(scale_factor.value))
    ...
    def check_model_potential_workchains(self):
        ...
        for ii in range(self.inputs.model_iterations_required):
            ...
            if not model_workchain.is_finished_ok:
                self.report(
                    'Model potential workchain for scale factor {} failed with status {}'.format(
                        model_workchain.scale_factor,
    ...
    def compute_dft_difference_potential(self):
        ...
        """
        self.ctx.v_defect_q_q0 = get_potential_difference(self.inputs.v_defect_q, self.inputs.v_defect_q0)
    ...
    def check_alignment_workchains(self):
        ...
        if not alignment_wc.is_finished_ok:
            self.report(
                'Potential alignment workchain (defect q=0 to host) failed with status {}'.format(alignment_wc.exit_status)
            )
        ...
        if not alignment_wc.is_finished_ok:
            self.report(
                'Potential alignment workchain (DFT diff to model) failed with status {}'.format(alignment_wc.exit_status)
            )
            return self.exit_codes.ERROR_SUB_PROCESS_FAILED_ALIGNMENT
        else:
    ...
    def get_isolated_energy(self):
        ...
        self.report("Fitting the model energies to obtain the model energy for the isolated case")
        self.ctx.isolated_energy = fit_energies(
            orm.Dict(dict=linear_dimensions),
            orm.Dict(dict=self.ctx.model_energies)
        )
        self.report("The isolated model energy is {} eV".format(self.ctx.isolated_energy.value*hartree_to_ev))
    ...
    def get_model_corrections(self):
        ...
        for scale_factor, model_energy in self.ctx.model_energies.items():
            self.ctx.model_correction_energies[scale_factor] = calc_correction(self.ctx.isolated_energy, model_energy)
    ...
    def compute_correction(self):
        ...
        total_correction = get_correction_energy(
            ...
            self.ctx.alignment_dft_to_model, 
            self.ctx.alignment_q0_to_host, 
            self.inputs.defect_charge
        )
    
        self.report('The computed correction, including potential alignments, is {} eV'.format(total_correction.value*hartree_to_ev))
