...
class PotentialAlignmentLanyZunger(WorkChain):
    ...
    def validate_inputs(self):
        ...
        if self.inputs.run_pw_host == Bool(False):
            if not ('host_parent_calculation' in self.inputs or 'host_parent_folder' in self.inputs):
                self.abort_nowait('Neither the parent_calculation nor the parent_folder for the host input was defined')
...
#         return bool('host_fftgrid' not in self.inputs)
    ...
    def run_atomic_sphere_pot_host(self):
        ...
        for i in self.inputs.host_structure.sites:
            ...
            self.report('Launching PpWorkChain for atom {} in the host system. pk value {}'.format(i,future.pid))
    ...
    def extract_values_host(self):
        ...
        for site in self.inputs.host_structure.sites:
            file_tmp = self.ctx[str(site)]['retrieved'].get_file_content('aiida.out').splitlines()
            ...
            for line in file_tmp:
                if 'Min, Max, imaginary charge:' in line:
                    self.ctx.atom_sphere_pot_host[str(symbol)+'_'+str(site.position)] = [i for i in line.strip().split(' ') if i != ''][4]
    ...
    def run_atomic_sphere_pot_defect(self):
        ...
        tmp_pp = self.ctx.defect_ppcalc.out.retrieved.get_file_content('aiida.filplot')
        ...
        for i in self.inputs.defect_structure.sites:
            ...
            inputs['parameters_pp'] = ParameterData(dict=parameters_pp)
            future = submit(PpWorkChain,**inputs)
            self.report('Launching PpWorkChain for atom {} in defective system. pk value {}'.format(i,future.pid))
            calcs[str(i)] = Outputs(future)
    ...
    def extract_values_defect(self):
        ...
        for site in self.inputs.defect_structure.sites:
            ...
            for line in file_tmp:
                if 'Min, Max, imaginary charge:' in line:
                    self.ctx.atom_sphere_pot_def[str(symbol)+'_'+str(site.position)] = [i for i in line.strip().split(' ') if i != ''][4]
