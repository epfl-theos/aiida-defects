...
class PotentialAlignmentWorkchain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input('alignment_scheme', valid_type=orm.Str, default=orm.Str('lany-zunger'))
    ...
    def setup(self):
        ...
        first_potential_shape = self.inputs.first_potential.get_shape(self.inputs.first_potential.get_arraynames()[0])
        second_potential_shape = self.inputs.second_potential.get_shape(self.inputs.second_potential.get_arraynames()[0])
        if first_potential_shape != second_potential_shape:
            if self.inputs.interpolate:
                self.ctx.interpolation_required = True
            else:
                self.report('The two potentials could not be aligned as they are the different shapes and interpolation is not allowed.')
        ...
        if self.inputs.alignment_scheme not in ['lany-zunger']:
                self.report('The requested alignment scheme, "{}" is not recognised.'.format(self.inputs.alignment_scheme))
    ...
    def results(self):
        ...
        Collect results
        """
        self.report("Completed alignment. An alignment of {} eV is required".format(self.ctx.alignment.value))
