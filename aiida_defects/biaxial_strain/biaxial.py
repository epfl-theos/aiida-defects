...
class BiaxialStrainWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input(
            "relax_axis", valid_type=Str, required=False, default=Str('c'))
    ...
    def run_pw(self):
        ...
        for value in self.inputs.strain:
            structure_tmp = biaxial_strain_structure(self.inputs.structure,
                                                     self.inputs.relax_axis,
                                                     Float(value))
            suitable_inputs = create_suitable_inputs_noclass(
                structure_tmp, self.inputs.magnetic_phase, self.inputs.B_atom)
            ...
            self.report(
                'Launching PwBaseWorkChain for the {}%  strain value. pk value {}'
                .format(value, future.pid))
