...
class PhonopyWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input(
            "kpoints", valid_type=KpointsData
        )  #It should be the one to be used for the bulk not supercell
        ...
        spec.input(
            'optimization', valid_type=Str
        )  #make possible to choose between vc-relax and relax
        spec.input(
            'opt_modulated',
            valid_type=Str,
            required=False,
    ...
    def optimize(self):
        ...
        if 'relax' in str(self.inputs.optimization):
            ...
            suitable_inputs = create_suitable_inputs_noclass(
                self.inputs.structure, self.inputs.magnetic_phase,
                self.inputs.B_atom)
            ...
            if str(self.inputs.magnetic_phase) in magnetic_phases:
                param['SYSTEM']['starting_magnetization'] = suitable_inputs[
                    'starting_magnetization'].get_dict()
            ...
            if bool(hubbard_U) and len(hubbard_U) == 1:
                ...
                for site in inputs_opt['structure'].sites:
                    if site.kind_name[:2] == str(
                            self.inputs.B_atom
                    ) or site.kind_name[:1] == 'Q' or site.kind_name[:1] == 'J':
            ...
            self.report(
                'Launching PwBaseWorkChain for the bulk optimization. pk value {}'
                .format(running.pid))
    ...
    def create_supercells_with_displacements(self):
        ...
        if "relax" in str(self.inputs.optimization):
            ...
            inline_params = {
                "structure":
                self.ctx.optimization.get_outputs_dict()['output_structure'],
                "phonopy_input":
                self.inputs.phonopy_input,
            }
        else:
            suitable_inputs = create_suitable_inputs_noclass(
                self.inputs.structure, self.inputs.magnetic_phase,
        ...
        self.ctx.disp_cells = create_supercells_with_displacements_inline(
            **inline_params)
    ...
    def run_pw_disp(self):
        ...
        kpoints_x = int(kpoints[0]) / int(
            self.inputs.phonopy_input.get_dict()['supercell'][0][0])
        kpoints_y = int(kpoints[1]) / int(
            self.inputs.phonopy_input.get_dict()['supercell'][1][1])
        kpoints_z = int(kpoints[2]) / int(
            self.inputs.phonopy_input.get_dict()['supercell'][2][2])
        ...
        for structure in self.ctx.disp_cells:
            suitable_inputs = create_suitable_inputs_noclass(
                self.ctx.disp_cells[structure], self.inputs.magnetic_phase,
                self.inputs.B_atom)
            ...
            self.report(
                'Launching PwBaseWorkChain for the {} with structure with displacements. pk value {}'
                .format(structure, future.pid))
    ...
    def force_constant_calculation(self):
        ...
        """
        if "relax" in str(self.inputs.optimization):
            self.ctx.phpy_input_structure = self.ctx.optimization.get_outputs_dict(
            )['output_structure']
        else:
            self.ctx.phpy_input_structure = self.ctx.structure

        self.ctx.inline_params = {
            "structure": self.ctx.phpy_input_structure,
            "phonopy_input": self.inputs.phonopy_input,
        }
        for structure in self.ctx.disp_cells:
            self.ctx.inline_params['force_' + structure.split('_')
                                   [1]] = self.ctx[structure]["output_array"]

        self.ctx.phonopy_data = get_force_constants_inline(
            **self.ctx.inline_params)
        self.ctx.inline_params['force_constants'] = self.ctx.phonopy_data[
            'phonopy_output']
    ...
    def create_modulations(self):
        ...
        """
        self.ctx.modulated_structures = modulations_inspection(
            **self.ctx.inline_params)

        if 'angle_scan' in self.ctx.modulated_structures:
            self.ctx.ops = list(self.ctx.modulated_structures['angle_scan'].
                                get_dict().values())
            self.out('ops_symmetries',
                     self.ctx.modulated_structures['angle_scan'])
        ...
        if not bool(self.ctx.modulated_structures):
            ...
            self.report(
                'Mode with degeneracy higher than two. Not able to treat it')
            self.ctx.modulated_structures = {
                key: val
                for key, val in self.ctx.modulated_structures.items()
                if val != Bool(False)
            }
        ...
        for modulation in self.ctx.modulated_structures:
            if 'nondeg' in modulation:
                self.ctx.non_degenerate[
                    modulation] = self.ctx.modulated_structures[modulation]
        ...
        for key in self.ctx.non_degenerate:
            if 'nondeg' in key and int(
                    key.split('_')[1]) not in self.ctx.nondeg_modes:
...
##############################################
    ...
    def amplitude_scan_nondegenerate(self):
        ...
        kpoints = self.inputs.kpoints.get_kpoints_mesh()[0]
        kpoints_x = int(kpoints[0]) / int(self.inputs.phonopy_input.get_dict()
                                          ['modulation']['supercell'][0][0])
        kpoints_y = int(kpoints[1]) / int(self.inputs.phonopy_input.get_dict()
                                          ['modulation']['supercell'][1][1])
        kpoints_z = int(kpoints[2]) / int(self.inputs.phonopy_input.get_dict()
                                          ['modulation']['supercell'][2][2])
        ...
        for label, structure in six.iteritems(self.ctx.non_degenerate):
            if not bool(self.ctx.repeat_nondeg) or self.ctx.repeat_nondeg[str(
                    label.split('_')[1]) + str(label.split('_')[3])] == False:
                suitable_inputs = create_suitable_inputs_noclass(
                    structure, self.inputs.magnetic_phase, self.inputs.B_atom)
                inputs['structure'] = suitable_inputs['structure']
                ...
                if str(self.inputs.magnetic_phase) in magnetic_phases:
                    param['SYSTEM'][
                        'starting_magnetization'] = suitable_inputs[
                            'starting_magnetization'].get_dict()
                    param['SYSTEM']['nspin'] = 2
                    ...
                    for site in inputs['structure'].sites:
                        if site.kind_name[:2] == str(
                                self.inputs.B_atom
                                            ...
                                            1] == 'Q' or site.kind_name[:
                                                                        1] == 'J':
                            U[str(site.kind_name)] = hubbard_U[list(hubbard_U)
                                                               [0]]
                    param['SYSTEM']['hubbard_u'] = U
                elif bool(hubbard_U) and len(hubbard_U) > 1:
                    param['SYSTEM']['lda_plus_u'] = True
                ...
                future = submit(PwBaseWorkChain, **inputs)
                self.report(
                    'Launching PwBaseWorkChain for the {} modulation. pk value {}'
                    .format(label, future.pid))
        ...
        for mode in self.ctx.nondeg_modes:
            ...
            for label, value in six.iteritems(self.ctx.non_degenerate):
                if "nondeg_" + str(mode) in label:
                    self.ctx.E_nondeg['nondeg_mode' +
                                      str(mode)][str(label)] = Float(
                                          self.ctx[str(label)]
    ...
    def fitting_scan_energies_nondegenerate(self):
        ...
        for mode in self.ctx.nondeg_modes:
            for label, energy in six.iteritems(
                    self.ctx.E_nondeg['nondeg_mode' + str(mode)]):
                amplitude.append(label.split('_')[3])
                energies.append(energy)

            ref_E = float(self.ctx.E_nondeg['nondeg_mode' + str(mode)][
                'nondeg_' + str(mode) + '_ampl_0'])
            ...
            name_file = str(self.inputs.structure.pk) + '_q_' + str(
                q) + '_mode_' + str(mode)
            ...
            if r2 >= 0.9:
                ...
                self.ctx.opt_nondegenerate[str(mode)] = modulations_optimal(
                    **self.ctx.inline_params)
            ...
            if r2 < 0.9:
                ...
                name_file = str(self.inputs.structure.pk) + '_q_' + str(
                    q) + '_mode_' + str(mode)
                popt, pcov = poly_fit(amplitudes, DeltaE, quadratic_poly,
                                      name_file)
                if popt[a] < 0:
                    ampl_min = self.inputs.phonopy_input.get_dict(
                    )['modulation']['amplitude'][0] * 10
                    ampl_max = self.inputs.phonopy_input.get_dict(
                    )['modulation']['amplitude'][1] * 10
                    ampl_incr = self.inputs.phonopy_input.get_dict(
                    )['modulation']['amplitude'][2] * 10
                    amplitudes = np.arange(ampl_min, ampl_max,
                                           ampl_incr).tolist()
                    ...
                    for ampl in amplitudes:
                        ...
                        ph_md = [[
                            ...
                            int(mode),
                            float(ampl),
                            self.inputs.phonopy_input.get_dict()['modulation']
                            ['phase']
                        ]]
                        ...
                        new_modulated_s = modulations_optimal(
                            **self.ctx.inline_params)

                        self.ctx.non_degenerate['nondeg_' + str(
                            mode) + '_ampl_' + str(ampl)] = new_modulated_s[
                                self.ctx.inline_params['name']]
                if popt[a] > 0:
                    ampl_min = self.inputs.phonopy_input.get_dict(
                    )['modulation']['amplitude'][0] / 10
                    ampl_max = self.inputs.phonopy_input.get_dict(
        ...
        self.ctx.repeat_nondeg = dict(
            list(zip(self.ctx.nondeg_modes, self.ctx.fitting_nondeg_ok)))
    ...
    def optimization_nondegenerate(self):
        ...
        for mode in self.ctx.opt_nondegenerate:
            for label, structure in six.iteritems(
                    self.ctx.opt_nondegenerate[mode]):
        ...
        for label, structure in six.iteritems(self.ctx.opt_nondegenerate_flat):
            ...
            future = submit(PwBaseWorkChain, **inputs)
            self.report(
                'Launching PwBaseWorkChain for the modulation along {} mode. pk value {}'
                .format(label, future.pid))
            calcs[label] = Outputs(future)
...
##############################################
    ...
    def amplitude_scan_degenerate(self):
        ...
        for label, structure in six.iteritems(self.ctx.degenerate):
            if not bool(self.ctx.repeat_deg) or self.ctx.repeat_deg[str(
                    label.split('_')[1]) + str(label.split('_')[3])] == False:
                suitable_inputs = create_suitable_inputs_noclass(
                    structure, self.inputs.magnetic_phase, self.inputs.B_atom)
                inputs['structure'] = suitable_inputs['structure']
    ...
    def retrieve_scan_energies_degenerate(self):
        ...
        for mode in self.ctx.deg_modes:
            for opd in self.ctx.ops:
                self.ctx.E_deg['DEG_mode' + str(mode) + '_ang_' +
                               str(opd)] = {}
                for label, value in six.iteritems(self.ctx.degenerate):
                    if "DEG_" + str(mode) + '_ang_' + str(opd) in label:
                        self.ctx.E_deg['DEG_mode' + str(mode) + '_ang_' + str(
                            opd)][str(label)] = self.ctx[str(
                                label)]["output_parameters"].dict.energy
    ...
    def fitting_scan_energies_degenerate(self):
        ...
        for mode in self.ctx.deg_modes:
            for opd in self.ctx.ops:
                ...
                ref_E = float(
                    ...
                    ['DEG_' + str(mode) + '_ang_' + str(opd) + '_ampl_0'])
                energies = [float(i) for i in energies]
                DeltaE = np.array(energies)
                DeltaE = DeltaE - ref_E
                #DeltaE[:] = [E - min_E for E in energies]
                ...
                #Try fitting quartic function
                name_file = str(self.inputs.structure.pk) + '_q_' + str(
                    q) + '_mode_' + str(mode) + '_ang_' + str(opd)
                popt, pcov = poly_fit(amplitudes, DeltaE, quartic_poly,
                                      name_file)
                ...
                if r2 >= 0.9:
                    ...
                    ph_md = [[
                        q,
                        int(str(mode)[1]), ampl_min * cos(radians(int(opd))),
                        self.inputs.phonopy_input.get_dict()['modulation']
                        ['phase']
                    ],
                             [
                                 ...
                                 self.inputs.phonopy_input.get_dict()
                                 ['modulation']['phase']
                             ]]
                    ...
                    self.ctx.opt_degenerate[str(mode) + '_' +
                                            str(opd)] = modulations_optimal(
                                                **self.ctx.inline_params)
                ...
                if r2 < 0.9:
                    ...
                    del self.ctx.E_deg['DEG_mode' + str(mode) + '_ang_' +
                                       str(opd)]
                    w = {}
                    ...
                    name_file = name = str(
                        self.inputs.structure.pk) + '_q_' + str(
                            q) + '_mode_' + str(mode) + '_angle_' + str(opd)
                    popt, pcov = poly_fit(amplitudes, DeltaE, quadratic_poly,
                                          name_file)
                    if popt[a] < 0:
                        ampl_min = self.inputs.phonopy_input.get_dict(
                        ...
                        for ampl in amplitudes:
                            ...
                            self.ctx.degenerate[
                                'DEG_' + str(mode) + '_ang_' + str(opd) +
                                '_ampl_' + str(ampl)] = new_modulated_s[
                                    self.ctx.inline_params['name']]
                    if popt[a] > 0:
                        ampl_min = self.inputs.phonopy_input.get_dict(
                        )['modulation']['amplitude'][0] / 10
        ...
        self.ctx.repeat_deg = dict(
            list(zip(self.ctx.deg_modes_ops, self.ctx.fitting_deg_ok)))
    ...
    def retrieve_results_degenerate(self):
        ...
        for label in self.ctx.E_deg:
            ...
            for key in self.ctx.E_deg[label]:
                E_deg[label.split('.')[0]][key.split('.')[0] + key.split(
                    '.')[1][1:]] = self.ctx.E_deg[label][key]

        self.out('energy_deg', ParameterData(dict=E_deg))
        self.ctx.res_deg = {}
        for label in self.ctx.opt_degenerate_flat:
            self.ctx.res_deg[label[:-2].replace('.', ';')] = {}
            self.ctx.res_deg[label[:-2].replace('.', ';')]['energy'] = Float(
                self.ctx[str(label)]["output_parameters"].dict.energy)
            self.ctx.res_deg[label[:-2].replace(
                '.',
                ';')]['structure'] = self.ctx[str(label)]["output_structure"]
            self.ctx.res_deg[label[:-2].replace('.', ';')]['symmetry'] = Int(
                get_spacegroup(self.ctx.res_deg[label[:-2].replace(
                    '.', ';')]['structure']))
            #self.out('modulation_'+str(label),self.ctx.res_nondeg[str(label)])

            for link_name, node in six.iteritems(
                    self.ctx.res_deg[label[:-2].replace('.', ';')]):
                self.out(link_name, node)
