...
def create_supercells_with_displacements_inline(**kwargs):
    ...
    https://github.com/ConradJohnston/aiida-defectslcarreras/aiida_extensions/tree/master/workflows
    Note:
...
def phonon_band_structure(**kwargs):
    ...
    phonon.plot_band_structure(symbols=q_labels).savefig(
        str(structure.pk) + '.pdf',
        dpi=300,
        transparent=True,
...
def modulations_inspection(**kwargs):
    ...
    if len(ndeg_band_indexes) > 0:
        for index in ndeg_band_indexes:
            for ampl in amplitudes:
                phonon_modes.append(
                    [q, index, ampl, phonopy_input['modulation']['phase']])

                non_degenerate = create_modulations(dm, dimension,
                                                    phonon_modes)
                modulated_structures['nondeg_' + str(index) + '_ampl_' + str(
                    ampl)] = non_degenerate['modulated_overlap']
    ...
    if len(deg_band_index) == 2:
        ...
        structures = []
        phonon_modes_d = [[
            q, deg_band_index[0], ampl * cos(radians(angle)),
            phonopy_input['modulation']['phase']
        ],
                          [
                              q, deg_band_index[1], ampl * sin(radians(angle)),
                              phonopy_input['modulation']['phase']
                          ]]

        degenerate = create_modulations(dm, dimension, phonon_modes_d)
        ...
        if len(list(set(modulated_symmetry))) == 4:
            ...
            while (not order_parameter_space and step < max_step):
                ...
                if len(list(set(modulated_symmetry))) < 4:
                    ...
                    etol = etol * 0.1
                    for structure in ms:
                        modulated_symmetry.append(
                            get_spacegroup(structure, etol))
                    ...
                    plt.title('symprec ' + str(etol))
                    plt.colorbar()
                    plt.savefig('Deg_Modulations_Scan_' + str(structureb.pk) +
                                '.pdf')
                    plt.show()
        ...
        for opd in ops:
            for ampl in amplitudes:
                phonon_modes_d = [[
                    q, deg_band_index[0], ampl * cos(radians(opd)),
                    phonopy_input['modulation']['phase']
                ],
                                  [
                                      q, deg_band_index[1],
                ...
                degenerate = create_modulations(dm, dimension, phonon_modes_d)
                modulated_structures[
                    'DEG_' + str(deg_band_index) + '_ang_' + str(opd) +
                    '_ampl_' + str(ampl)] = degenerate['modulated_overlap']
...
def modulations_optimal(**kwargs):
    ...
    modulated_structures = {}
    modulation = create_modulations(dm, dimension, phonon_modes)
