...
corrections_default = ParameterData(dict={
    'makov_payne': False,
    'bandfilling': False,
...
class DefectWorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input(
            'kpoints_force_parity', valid_type=Bool, default=Bool(False))
        ...
        spec.input(
            'max_meta_convergence_iterations', valid_type=Int, default=Int(5))
        spec.input('meta_convergence', valid_type=Bool, default=Bool(True))
        spec.input(
            'relaxation_scheme', valid_type=Str, default=Str('vc-relax'))
        ...
        spec.input(
            ...
            valid_type=List,
            required=False,
            default=vacancies_default)
        spec.input(
            'substitutions',
            valid_type=ParameterData,
            required=False,
            default=substitutions_default)
        spec.input(
            'scale_sc',
            valid_type=List,
            required=False,
            default=scale_sc_default)
        spec.input(
            'cluster', valid_type=Bool, required=False, default=Bool(False))
        spec.input(
            'defect_charge',
            valid_type=Float,
            required=False,
        ...
        spec.input(
            'corrections',
            valid_type=ParameterData,
            default=corrections_default)
        spec.input(
            'epsilon_r', valid_type=Float, required=False, default=Float(1.00))
        spec.input('pot_align', valid_type=Float, required=False)
        spec.input_group('bf_relax')

        spec.outline(
            if_(cls.should_run_primitive_host)(cls.run_primitive_host,
                                               cls.retrieve_host_results),
            cls.create_defective_supercells,
            cls.run_defective_structures,
            cls.retrieve_defect_results,
            if_(cls.should_run_makovpayne)(cls.run_makovpayne),
            if_(cls.should_run_bandfilling)(cls.run_bandfilling),
            if_(cls.should_run_pot_align)(cls.run_pot_align),
    ...
    def run_primitive_host(self):
        ...
        if self.inputs.type_run_primitive_host == 'scf':
            ...
            if not ('kpoints' in self.inputs):
                self.abort_nowait(
                    'You need to provide a kpoint inputs when type_run_primitive_host = scf'
                )
            ...
            self.report(
                'Launching PwBaseWorkChain for the stoichiometric host. pk value {}'
                .format(running.pid))
            ...
            if 'kpoints_force_parity' in self.inputs:
                inputs[
                    'kpoints_force_parity'] = self.inputs.kpoints_force_parity

            other_inputs = [
                'vdw_table', 'final_scf', 'group', 'max_iterations',
                'max_meta__convergence_iterations', 'meta_convergence',
                'volume_convergence', 'clean_workdir'
            ]
            ...
            self.report(
                'Launching PwRelaxWorkChain for the host system. pk value {}'.
                format(running.pid))
            return ToContext(host_pwcalc=running)
    ...
    def create_defective_supercells(self):
        ...
        self.ctx.defective_structures = defect_creator(
            self.ctx.structure_host, vacancies, substitutions, scale_sc,
            cluster)
        ...
        if self.inputs.type_run_defects == 'scf':
            ...
            for structure in self.ctx.defective_structures:
                if '_0' not in structure:
                    ...
                    if not ('kpoints' in self.inputs):
                        self.abort_nowait(
                            'You need to provide a kpoint inputs when type_run_defects = scf'
                        )

                    param = self.inputs.parameters.get_dict()
                    param['SYSTEM']['tot_charge'] = float(
                        self.inputs.defect_charge)

                    inputs['parameters'] = ParameterData(dict=param)
                    inputs['structure'] = self.ctx.defective_structures[
                        structure]
                    inputs['kpoints'] = self.inputs.kpoints
                    ...
                    self.report(
                        'Launching PwBaseWorkChain for the defective structure {} with charge {}. pk value {}'
                        .format(structure, self.inputs.defect_charge,
                                future.pid))
            ...
            for structure in self.ctx.defective_structures:
                if '_0' not in structure:
                    ...
                    self.report(
                        'Launching PwRelaxWorkChain for the defective structure {} with charge {}. pk value {}'
                        .format(structure, self.inputs.defect_charge,
                                future.pid))
                    calcs[str(structure)] = Outputs(future)
    ...
    def retrieve_defect_results(self):
        ...
        if self.inputs.type_run_defects == 'vc-relax' or self.inputs.type_run_defects == 'relax':
            for structure in self.ctx.defective_structures:
                if '_0' not in structure:
                    self.ctx.optimized_defective_structures[
                        structure] = self.ctx[structure]['output_structure']
        else:
            self.ctx.optimized_defective_structures = self.ctx.defective_structures

        for structure in self.ctx.defective_structures:
            if '_0' not in structure:
                self.out('structure_' + str(structure),
                         self.ctx.optimized_defective_structures[structure])
                self.out(
                    'energy_' + str(structure),
                    Float(
                        self.ctx[structure]["output_parameters"].dict.energy))
    ...
    def should_run_makovpayne(self):
        ...
        return bool(self.inputs.corrections.get_dict()['makov_payne']
                    and self.inputs.defect_charge != 0.0)
    ...
    def run_makovpayne(self):
        ...
        """
        inputs = {
            'bulk_structure': self.ctx.structure_host,
        ...
        self.report('Launching MakovPayneCorrection. pk value {}'.format(
            running.pid))
    ...
    def should_run_pot_align(self):
        ...
        if bool(self.inputs.corrections.get_dict()['potential_alignment']):
            run_pot_align = True
        elif bool(self.inputs.corrections.get_dict()['bandfilling']
                  ) and self.inputs.pot_align not in self.inputs:
            run_pot_align = True
        else:
            run_pot_align = False
    ...
    def run_pot_align(self):
        ...
        inputs = {
            'host_structure': self.ctx.structure_host,
        ...
        for structure in self.ctx.optimized_defective_structures:
            ...
            if '_0' not in structure:
                inputs[
                    'defect_structure'] = self.ctx.optimized_defective_structures[
                        structure]
                #inputs['defect_parent_folder'] = self.ctx[structure]['remote_folder'] #self.ctx[structure].inp.parameters
                inputs[
                    'parameters_pw_defect'] = self.ctx.inputs_defective_structures[
                        structure]['parameters']

                future = submit(PotentialAlignmentLanyZunger, **inputs)
                self.report(
                    'Launching PotentialAlignmentLanyZunger for the defective structure {} with charge {}. pk value {}'
                    .format(structure, self.inputs.defect_charge, future.pid))
            ...
            if '_0' not in structure:
                inputs[
                    'defect_structure'] = self.ctx.optimized_defective_structures[
                        structure]
                inputs[
                    'defect_parameters'] = self.ctx.inputs_defective_structures[
                        structure][
                            'parameters']  #self.ctx[structure].inp.parameters

                future = submit(BandFillingCorrectionWorkChain, **inputs)
                self.report(
                    'Launching BandFillingCorrectionWorkChain for the defective structure {} with charge {}. pk value {}'
                    .format(structure, self.inputs.defect_charge, future.pid))
    ...
    def retrieve_corrections(self):
        ...
        if bool(self.inputs.corrections.get_dict()['makov_payne']
                and self.inputs.defect_charge != 0.0):
            self.out('makov_payne',
                     self.ctx.makov_payne.out.Makov_Payne_Correction)

        if bool(
                self.inputs.corrections.get_dict()['potential_alignment']
        ):  #or bool(self.inputs.corrections.get_dict()['bandfilling']):
            for structure in self.ctx.optimized_defective_structures:
                if '_0' not in structure:
                    self.out(
                        'pot_align' + str(structure),
                        self.ctx['pot_align_' + str(structure)].out.pot_align *
                        self.inputs.defect_charge)
        ...
        if bool(self.inputs.corrections.get_dict()['bandfilling']):
            for structure in self.ctx.optimized_defective_structures:
                if '_0' not in structure:
                    self.out(
                        'E_donor' + str(structure),
                        self.ctx['bandfilling_' + str(structure)]["E_donor"])
                    self.out(
                        'E_acceptor' + str(structure),
                        self.ctx['bandfilling_' +
                                 str(structure)]["E_acceptor"])
